...
theorem bsearch_is_strict_mono : (bSearch α).IsStrictMono := by
  ...
  . intros q hq i hi h_ready
    ...
    . simp
     ...
     -- rw [Prod.Lex.le_iff (q.target, q.found) i] at hi
      

      /-
      case right.inl.inr.inl
      α✝ : Type ?u.14648
      inst✝³ : Inhabited α✝
      inst✝² : LinearOrder α✝
      α : Type
      ...
      inst✝ : LinearOrder α
      q : (bSearch α).σ
      hq : Stream.valid (bSearch α) q
      i : StreamOrder α
      hi : (q.target, decide (q.found = true)) ≤ i
      r : Stream.ready (bSearch α) q
      h_skip_valid : Stream.valid (bSearch α) (Stream.skip (bSearch α) q hq i)
      h✝¹ : ¬i.fst > q.target
      h✝ : i.fst < q.target
      ⊢ False
      -/

      -- I think there's a contradiction between hi and the i.fst < q.target hypothesis
